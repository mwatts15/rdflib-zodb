...
class ZODBStore(Persistent, Store):
    ...
    def __makeSlices(self, items, isSorted=False):
        if not isSorted:
            items = sorted(items)
    ...
    def __exo(self, index, triple, context, tidx, aidx, bidx):
        ...
        if len(target) == 1:
            ...
            slices = self.__makeSlices(obj_ids)
            for min_id, max_id in slices:
                items = dict(index.items(min=min_id, max=max_id))
                for y in obj_ids:
    ...
    def __multiple_obj2id(self, objs):
        slices = self.__makeSlices(objs)
        items_list = []
        for least_obj, greatest_obj in slices:
            items = dict(self.__obj2int.items(min=least_obj, max=greatest_obj))
